...
func (api *PublicWhisperAPI) MarkTrustedPeer(ctx context.Context, vnode string) (bool, error) {
	n, err := vntp2p.ParseNode(vnode)
	if err != nil {
		return false, err
	}

	//ID := vntp2p.PeerIDtoNodeID(n.Id)
...
func (api *PublicWhisperAPI) Post(ctx context.Context, req NewMessage) (bool, error) {
	...
	if len(req.TargetPeer) > 0 {
		n, err := vntp2p.ParseNode(req.TargetPeer)
		if err != nil {
