...
func (w *Whisper) Protocols() []vntp2p.Protocol {
	return []vntp2p.Protocol{w.protocol}
}
...
func (w *Whisper) getPeer(peerID libp2p.ID) (*Peer, error) {
	w.peerMu.Lock()
...
func (w *Whisper) AllowP2PMessagesFromPeer(peerID libp2p.ID) error {
...
func (w *Whisper) RequestHistoricMessages(peerID libp2p.ID, envelope *Envelope) error {
	p, err := w.getPeer(peerID)
	if err != nil {
		return err
	}
...
func (w *Whisper) SendP2PMessage(peerID libp2p.ID, envelope *Envelope) error {
	p, err := w.getPeer(peerID)
	if err != nil {
		return err
	}
...
func (w *Whisper) SendP2PDirect(peer *Peer, envelope *Envelope) error {
	return vntp2p.Send(peer.ws, ProtocolName, p2pCode, envelope)
}
...
func (w *Whisper) Start(*vntp2p.Server) error {
...
func (w *Whisper) HandlePeer(peer *vntp2p.Peer, rw vntp2p.MsgReadWriter) error {
...
func (w *Whisper) runMessageLoop(p *Peer, rw vntp2p.MsgReadWriter) error {
	for {
		...
		if packet.GetBodySize() > w.MaxMessageSize() {
		...
		case messagesCode:
			...
			if err := packet.Decode(&envelope); err != nil {
				log.Warn("failed to decode envelope, peer will be disconnected", "peer", p.peer.RemoteID(), "err", err)
				return errors.New("invalid envelope")
			}
			cached, err := w.add(&envelope)
			if err != nil {
				log.Warn("bad envelope received, peer will be disconnected", "peer", p.peer.RemoteID(), "err", err)
				return errors.New("invalid envelope")
			}
			if cached {
				p.mark(&envelope)
