...
// Whisper represents a dark communication interface through the VNT
// network, using its very own P2P communication layer.
type Whisper struct {
	protocol vntp2p.Protocol // Protocol description and parameters
...
// New creates a Whisper client ready to communicate through the VNT P2P network.
func New(cfg *Config) *Whisper {
	...
	whisper.protocol = vntp2p.Protocol{
		Name:    ProtocolName,
...
func (whisper *Whisper) Protocols() []vntp2p.Protocol {
	return []vntp2p.Protocol{whisper.protocol}
}
...
func (whisper *Whisper) getPeer(peerID libp2p.ID) (*Peer, error) {
	whisper.peerMu.Lock()
...
func (whisper *Whisper) AllowP2PMessagesFromPeer(peerID libp2p.ID) error {
...
func (whisper *Whisper) RequestHistoricMessages(peerID libp2p.ID, envelope *Envelope) error {
	p, err := whisper.getPeer(peerID)
	if err != nil {
		return err
	}
	p.trusted = true
	return vntp2p.Send(p.ws, ProtocolName, p2pRequestCode, envelope)
}

// SendP2PMessage sends a peer-to-peer message to a specific peer.
func (whisper *Whisper) SendP2PMessage(peerID libp2p.ID, envelope *Envelope) error {
	p, err := whisper.getPeer(peerID)
	if err != nil {
		return err
	}
...
func (whisper *Whisper) SendP2PDirect(peer *Peer, envelope *Envelope) error {
	return vntp2p.Send(peer.ws, ProtocolName, p2pMessageCode, envelope)
}
...
func (whisper *Whisper) Start(*vntp2p.Server) error {
...
func (whisper *Whisper) HandlePeer(peer *vntp2p.Peer, rw vntp2p.MsgReadWriter) error {
...
func (whisper *Whisper) runMessageLoop(p *Peer, rw vntp2p.MsgReadWriter) error {
	for {
		...
		if packet.GetBodySize() > whisper.MaxMessageSize() {
			log.Warn("oversized message received", "peer", p.peer.RemoteID())
		...
		case statusCode:
			...
			log.Warn("unxepected status message received", "peer", p.peer.RemoteID())
		case messagesCode:
			// decode the contained envelopes
			var envelopes []*Envelope
			if err := packet.Decode(&envelopes); err != nil {
				log.Warn("failed to decode envelopes, peer will be disconnected", "peer", p.peer.RemoteID(), "err", err)
			...
			for _, env := range envelopes {
				...
				if err != nil {
					trouble = true
					log.Error("bad envelope received, peer will be disconnected", "peer", p.peer.RemoteID(), "err", err)
				}
				if cached {
					p.mark(env)
				}
		...
		case powRequirementCode:
			...
			i, err := s.Uint()
			if err != nil {
				log.Warn("failed to decode powRequirementCode message, peer will be disconnected", "peer", p.peer.RemoteID(), "err", err)
				return errors.New("invalid powRequirementCode message")
			}
			f := math.Float64frombits(i)
			if math.IsInf(f, 0) || math.IsNaN(f) || f < 0.0 {
				log.Warn("invalid value in powRequirementCode message, peer will be disconnected", "peer", p.peer.RemoteID(), "err", err)
		...
		case bloomFilterExCode:
			...
			if err != nil {
				log.Warn("failed to decode bloom filter exchange message, peer will be disconnected", "peer", p.peer.RemoteID(), "err", err)
		...
		case p2pMessageCode:
			...
			if p.trusted {
				...
				if err := packet.Decode(&envelope); err != nil {
					log.Warn("failed to decode direct message, peer will be disconnected", "peer", p.peer.RemoteID(), "err", err)
		...
		case p2pRequestCode:
			...
			if whisper.mailServer != nil {
				...
				if err := packet.Decode(&request); err != nil {
					log.Warn("failed to decode p2p request message, peer will be disconnected", "peer", p.peer.RemoteID(), "err", err)
