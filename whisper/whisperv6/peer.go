...
func (peer *Peer) handshake() error {
	...
	go func() {
		...
		errc <- vntp2p.SendItems(peer.ws, ProtocolName, statusCode, ProtocolVersion, powConverted, bloom)
	}()
...
func (peer *Peer) broadcast() error {
	...
	if len(bundle) > 0 {
		...
		if err := vntp2p.Send(peer.ws, ProtocolName, messagesCode, bundle); err != nil {
			return err
		}
...
func (peer *Peer) notifyAboutPowRequirementChange(pow float64) error {
	...
	return vntp2p.Send(peer.ws, ProtocolName, powRequirementCode, i)
}

func (peer *Peer) notifyAboutBloomFilterChange(bloom []byte) error {
	return vntp2p.Send(peer.ws, ProtocolName, bloomFilterExCode, bloom)
}
