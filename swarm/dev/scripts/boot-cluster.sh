...
#
# The cluster contains a bootnode, a gvnt node and multiple swarm nodes, with
...
BOOTNODE_URL="vnode://${BOOTNODE_PUBKEY}@${BOOTNODE_IP}:${BOOTNODE_PORT}"

# static gvnt configuration
GVNT_IP="192.168.33.3"
GVNT_RPC_PORT="8545"
GVNT_RPC_URL="http://${GVNT_IP}:${GVNT_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gvnt and
# swarm nodes
start_bootnode() {
...
# start_vnt_node starts a gvnt node with --datadir pointing at <base-dir>/gvnt
# and a single, unlocked account with password "gvnt"
start_vnt_node() {
  local dir="${base_dir}/gvnt"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gvnt account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GVNT_IP}"
    --rpcport   "${GVNT_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gvnt" "${GVNT_IP}" "$(which gvnt)" ${args[@]}
}
...
create_account() {
  ...
  gvnt --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
