...
/*
bzz implements the swarm wire protocol [bzz] (sister of vnt and shh)
...
import (
	...
	bzzswap "github.com/vntchain/go-vnt/swarm/services/swap"
	"github.com/vntchain/go-vnt/swarm/services/swap/swap"
...
type bzz struct {
	...
	peer       *vntp2p.Peer         // the p2p peer object
	rw         vntp2p.MsgReadWriter // messageReadWriter to send messages to
...
*/
func Bzz(cloud StorageHandler, backend chequebook.Backend, hive *Hive, dbaccess *DbAccess, sp *bzzswap.SwapParams, sy *SyncParams, networkId uint64) (vntp2p.Protocol, error) {
	...
	if err != nil {
		return vntp2p.Protocol{}, fmt.Errorf("error setting up request db: %v", err)
	}
	fmt.Println(requestDb)
	if networkId == 0 {
		networkId = NetworkId
	}
	return vntp2p.Protocol{
		Name:    "bzz",
		Version: Version,
...
*/
func run(requestDb *storage.LDBDatabase, depo StorageHandler, backend chequebook.Backend, hive *Hive, dbaccess *DbAccess, sp *bzzswap.SwapParams, sy *SyncParams, networkId uint64, p *vntp2p.Peer, rw vntp2p.MsgReadWriter) (err error) {
...
func (self *bzz) Drop() {
	self.peer.Disconnect(vntp2p.DiscSubprotocolError)
}
...
func (self *bzz) handle() error {
	...
	if size > ProtocolMaxMsgSize {
		return fmt.Errorf("message too long: %v > %v", size, ProtocolMaxMsgSize)
	}
	...
	default:
		...
		return fmt.Errorf("invalid message code: %v", msg.Body.Type)
	}
	return nil
}
...
func (self *bzz) handleStatus() (err error) {
	...
	err = vntp2p.Send(self.rw, "bzz", statusMsg, handshake)
	if err != nil {
		return err
	}
	...
	if msg.Body.Type != statusMsg {
		return fmt.Errorf("first msg has code %x (!= %x)", msg.Body.Type, statusMsg)
	}
...
// returns self advertised node connection info (listening address w vnodes)
...
func (self *bzz) send(msg vntp2p.MessageType, data interface{}) error {
	if self.hive.blockWrite {
		return fmt.Errorf("network write blocked")
	}
	log.Trace(fmt.Sprintf("-> %v: %v (%T) to %v", msg, data, data, self))
	err := vntp2p.Send(self.rw, "bzz", msg, data)
	if err != nil {
		self.Drop()
	}
	return err
