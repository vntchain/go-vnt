...
import (
	...
	"github.com/vntchain/go-vnt/swarm/services/swap"
...
type peerAddr struct {
	IP   net.IP
	Port uint16
	ID   libp2p.ID // the 64 byte NodeID (ECDSA Public Key)
	Addr kademlia.Address
}

// peerAddr pretty prints as vnode
func (self *peerAddr) String() string {
	return vntp2p.NewNode(self.ID, self.IP, 0, self.Port).String()
}
