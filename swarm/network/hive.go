...
import (
	...
	"github.com/vntchain/go-vnt/swarm/network/kademlia"
...
// connectPeer is a function to connect to a peer based on its NodeID or vnode URL
// there are called on the p2p.Server which runs on the node
func (self *Hive) Start(id libp2p.ID, listenAddr func() string, connectPeer func(string) error) (err error) {
	...
	go func() {
		...
		for alive := range self.more {
			...
			if node != nil && len(node.Url) > 0 {
				...
				// vnode or any lower level connection address is unnecessary in future
