...
import (
	...
	httpapi "github.com/vntchain/go-vnt/swarm/api/http"
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := vntclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func (self *Swarm) Start(srv *vntp2p.Server) error {
	startTime = time.Now()
	...
	self.hive.Start(
		vntp2p.PubkeyID(&srv.PrivateKey.PublicKey),
...
// implements the node.Service interface
func (self *Swarm) Protocols() []vntp2p.Protocol {
