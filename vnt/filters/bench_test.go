...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/gvnt/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := vntdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = vntdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db vntdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*vntdb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db vntdb.Database) {
