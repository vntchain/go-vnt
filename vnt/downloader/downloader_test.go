...
type downloadTester struct {
	...
	stateDb vntdb.Database // Database used by the tester for syncing from peers
	peerDb  vntdb.Database // Database of the peers containing all data
	...
	peerHashes   map[libp2p.ID][]common.Hash                  // Hash chain belonging to different test peers
	peerHeaders  map[libp2p.ID]map[common.Hash]*types.Header  // Headers belonging to different test peers
	peerBlocks   map[libp2p.ID]map[common.Hash]*types.Block   // Blocks belonging to different test peers
	peerReceipts map[libp2p.ID]map[common.Hash]types.Receipts // Receipts belonging to different test peers
	peerChainTds map[libp2p.ID]map[common.Hash]*big.Int       // Total difficulties of the blocks in the peer chains

	peerMissingStates map[libp2p.ID]map[common.Hash]bool // State entries that fast sync should not return
...
func newTester() *downloadTester {
	...
	tester := &downloadTester{
		...
		peerHashes:        make(map[libp2p.ID][]common.Hash),
		peerHeaders:       make(map[libp2p.ID]map[common.Hash]*types.Header),
		peerBlocks:        make(map[libp2p.ID]map[common.Hash]*types.Block),
		peerReceipts:      make(map[libp2p.ID]map[common.Hash]types.Receipts),
		peerChainTds:      make(map[libp2p.ID]map[common.Hash]*big.Int),
		peerMissingStates: make(map[libp2p.ID]map[common.Hash]bool),
	}
	tester.stateDb = vntdb.NewMemDatabase()
...
// contains a transactio to allow testing correct block
// reassembly.
...
func (dl *downloadTester) sync(id libp2p.ID, td *big.Int, mode SyncMode) error {
	dl.lock.RLock()
...
func (dl *downloadTester) newPeer(id libp2p.ID, version int, hashes []common.Hash, headers map[common.Hash]*types.Header, blocks map[common.Hash]*types.Block, receipts map[common.Hash]types.Receipts) error {
...
func (dl *downloadTester) newSlowPeer(id libp2p.ID, version int, hashes []common.Hash, headers map[common.Hash]*types.Header, blocks map[common.Hash]*types.Block, receipts map[common.Hash]types.Receipts, delay time.Duration) error {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
func (dl *downloadTester) dropPeer(id libp2p.ID) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
func testMultiSynchronisation(t *testing.T, protocol int, mode SyncMode) {
	...
	for i := 0; i < targetPeers; i++ {
		id := libp2p.ID(fmt.Sprintf("peer #%d", i))
...
func testMultiProtoSync(t *testing.T, protocol int, mode SyncMode) {
	...
	if err := tester.sync(libp2p.ID(fmt.Sprintf("peer %d", protocol)), nil, mode); err != nil {
		t.Fatalf("failed to synchronise blocks: %v", err)
	}
	assertOwnChain(t, tester, targetBlocks+1)
	...
	for _, version := range []int{62, 63, 64} {
		peer := libp2p.ID(fmt.Sprintf("peer %d", version))
...
func testBlockHeaderAttackerDropping(t *testing.T, protocol int) {
	...
	for i, tt := range tests {
		...
		id := libp2p.ID(fmt.Sprintf("test %d", i))
		...
		tester.downloader.synchroniseMock = func(libp2p.ID, common.Hash) error { return tt.result }
...
func (ftp *floodingTestPeer) RequestHeadersByNumber(from uint64, count, skip int, reverse bool) error {
	...
	for i := 0; i < cap(deliveriesDone); i++ {
		peer := libp2p.ID(fmt.Sprintf("fake-peer%d", i))
		ftp.pend.Add(1)
