...
type peerConnection struct {
	id libp2p.ID // Unique identifier of the peer
	...
	version int        // Vnt protocol version number to switch strategies
...
func newPeerConnection(id libp2p.ID, version int, peer Peer, logger log.Logger) *peerConnection {
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [vnt/62+] requested on vnt/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [vnt/62+] requested on vnt/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [vnt/63+] requested on vnt/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [vnt/63+] requested on vnt/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
// download procedure.
type peerSet struct {
	peers        map[libp2p.ID]*peerConnection
...
func newPeerSet() *peerSet {
	return &peerSet{
		peers: make(map[libp2p.ID]*peerConnection),
	}
}
...
func (ps *peerSet) Peer(id libp2p.ID) *peerConnection {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
