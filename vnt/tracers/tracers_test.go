...
import (
	...
	"github.com/vntchain/go-vnt/tests"
	"github.com/vntchain/go-vnt/vntdb"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gvnt console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = vnt.getBlock(vnt.getTransaction(tx).blockHash);
  var genesis = vnt.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.vnt.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  vnt.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			context := vm.Context{
				...
				Origin:      origin,
				Coinbase:    test.Context.Producer,
