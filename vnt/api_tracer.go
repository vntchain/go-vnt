...
// TraceChain returns the structured logs created during the execution of WAVM
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.vnt.producer.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.vnt.blockchain.CurrentBlock()
	default:
		from = api.vnt.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.vnt.producer.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.vnt.blockchain.CurrentBlock()
	default:
		to = api.vnt.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabase(api.vnt.ChainDb())

	if number := start.NumberU64(); number > 0 {
		start = api.vnt.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					vmctx := core.NewVMContext(msg, task.block.Header(), api.vnt.blockchain, nil)
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.vnt.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.vnt.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
// WAVM and returns them as a JSON object.
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.vnt.producer.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.vnt.blockchain.CurrentBlock()
	default:
		block = api.vnt.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.vnt.blockchain.GetBlockByHash(hash)
	if block == nil {
...
// TraceBlock returns the structured logs created during the execution of WAVM
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.vnt.engine.VerifyHeader(api.vnt.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.vnt.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewVMContext(msg, block.Header(), api.vnt.blockchain, nil)
	...
	for i, tx := range txs {
		...
		msg, _ := tx.AsMessage(signer)
		vmctx := core.NewVMContext(msg, block.Header(), api.vnt.blockchain, nil)
		vmenv := core.GetVM(msg, vmctx, statedb, api.config, vm.Config{})
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	statedb, err := api.vnt.blockchain.StateAt(block.Root())
	if err == nil {
		return statedb, nil
	}
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.vnt.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.vnt.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
// TraceTransaction returns the structured logs created during the execution of WAVM
// and returns them as a JSON object.
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	// Retrieve the transaction and assemble its WAVM context
	tx, blockHash, _, index := rawdb.ReadTransaction(api.vnt.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	// Create the parent state database
	block := api.vnt.blockchain.GetBlockByHash(blockHash)
	if block == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewVMContext(msg, block.Header(), api.vnt.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := core.GetVM(msg, context, statedb, api.config, vm.Config{})
