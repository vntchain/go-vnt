...
type fetcherTester struct {
	...
	drops  map[libp2p.ID]bool           // Map of peers dropped by the fetcher
...
func (f *fetcherTester) dropPeer(peer libp2p.ID) {
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *fetcherTester) makeHeaderFetcher(peer libp2p.ID, blocks map[common.Hash]*types.Block, drift time.Duration) headerRequesterFn {
...
func (f *fetcherTester) makeBodyFetcher(peer libp2p.ID, blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	closure := make(map[common.Hash]*types.Block)
	for hash, block := range blocks {
		closure[hash] = block
	}
...
func TestDistantPropagationDiscarding(t *testing.T) {
	...
	high := len(hashes)/2 - maxQueueDist - 1
