...
// NewPublicVntAPI creates a new VNT protocol API.
func NewPublicVntAPI(b Backend) *PublicVntAPI {
	return &PublicVntAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicVntAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current VNT protocol version this node supports
func (s *PublicVntAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicVntAPI) Syncing() (interface{}, error) {
...
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Hubble Signed Message:\n%d%s", len(data), data)
...
//
// https://github.com/vntchain/go-vnt/wiki/Management-APIs#personal_sign
...
//
// https://github.com/vntchain/go-vnt/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid VNT signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the VNT blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new VNT blockchain API.
...
func (s *PublicBlockChainAPI) GetBlockByNumber(ctx context.Context, blockNr rpc.BlockNumber, fullTx bool) (map[string]interface{}, error) {
	...
	if block != nil {
		...
		if err == nil && blockNr == rpc.PendingBlockNumber {
			...
			for _, field := range []string{"hash", "producer"} {
				response[field] = nil
			}
		}
		return response, err
...
func (s *PublicBlockChainAPI) GetAbi(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (hexutil.Bytes, error) {
	state, _, err := s.b.StateAndHeaderByNumber(ctx, blockNr)
	if state == nil || err != nil {
		return nil, err
	}
...
func (s *PublicBlockChainAPI) GetAllCandidates(ctx context.Context) ([]rpc.Candidate, error) {
	...
	for i, ca := range list {
		...
		rpcCandidates[i].Registered = ca.Registered
		...
		rpcCandidates[i].Binder = ca.Binder.String()
		rpcCandidates[i].Beneficiary = ca.Beneficiary.String()
		rpcCandidates[i].Bind = ca.Bind
	}
...
func (s *PublicBlockChainAPI) GetRestVNTBounty(ctx context.Context) (*big.Int, error) {
	...
	if rest := election.QueryRestReward(stateDB, s.b.CurrentBlock().Number()); rest == nil {
...
func RPCMarshalBlock(b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	...
	fields := map[string]interface{}{
		...
		"witnesses":        head.Witnesses,
		"signature":        hexutil.Bytes(head.Signature),
		"CmtMsges":         head.CmtMsges,
	}
...
// PublicDebugAPI is the collection of VNT APIs exposed over the public
// debugging endpoint.
...
// TODO remove pow rpc api
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	block, _ := api.b.BlockByNumber(ctx, rpc.BlockNumber(number))
	if block == nil {
		return "", fmt.Errorf("block #%d not found", number)
	}
	return fmt.Sprintln("Pow API not support any more"), nil
}

// PrivateDebugAPI is the collection of VNT APIs exposed over the private
// debugging endpoint.
...
func NewPublicNetAPI(net *vntp2p.Server, networkVersion uint64) *PublicNetAPI {
