...
type testOdr struct {
	OdrBackend
	sdb, ldb vntdb.Database
	disable  bool
}

func (odr *testOdr) Database() vntdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db vntdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes1(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db vntdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db vntdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db vntdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db vntdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	var (
		sdb     = vntdb.NewMemDatabase()
		ldb     = vntdb.NewMemDatabase()
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, dpos.NewFaker(), sdb, 4, testChainGen)
	if _, err := blockchain.InsertChain(gchain); err != nil {
		t.Fatal(err)
	}

	odr := &testOdr{sdb: sdb, ldb: ldb}
	lightchain, err := NewLightChain(odr, params.TestChainConfig, dpos.NewFaker())
	if err != nil {
		t.Fatal(err)
	}
