...
import (
	...
	"github.com/vntchain/go-vnt/signer/core"
	"github.com/vntchain/go-vnt/signer/storage"
)
...
func (alwaysDenyUI) OnApprovedTx(tx vntapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx vntapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(vntapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := vntapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx vntapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
