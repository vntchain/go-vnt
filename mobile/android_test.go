...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gvnt.LightScryptN, Gvnt.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.vnt", new NodeConfig());
			node.start();
			...
			VNTClient ec = node.getVNTClient();
	...
	// signatures too. Regression test for go-vnt issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gvnt-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/vntchain/gvnttest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gvnt and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.vntchain", "github.com/vntchain/go-vnt/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gvnt.aar"), "gvnt.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "vntchain", "gvnttest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
