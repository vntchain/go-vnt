...
type ChainIndexer struct {
	chainDb  vntdb.Database      // Chain database to index the data from
	indexDb  vntdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb, indexDb vntdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
...
func (c *ChainIndexer) eventLoop(currentHeader *types.Header, events chan ChainEvent, sub event.Subscription) {
	log.Debug("chain_indexer", "func", "eventLoop", "currentHeader", currentHeader)
