...
import (
	...
	"github.com/vntchain/go-vnt/common/mclock"
...
type BlockChain struct {
	...
	db     vntdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default VNT Validator and
// Processor.
func NewBlockChain(db vntdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) Stop() {
	...
	//  - HEAD-1:   So we don't do large reorgs if our HEAD becomes an uncle, never happen
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= vntdb.IdealBatchSize {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - vntdb.IdealBatchSize)
			}
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			// Block and stateDb both already known. However if the current block is below
		...
		case err == consensus.ErrPrunedAncestor:
			...
			// Import all the pruned blocks to make the stateDb available
			bc.chainmu.Unlock()
		...
		// Verify the witness list using the parent's state
		if err := bc.engine.VerifyWitnesses(block.Header(), stateDb, parent.Header()); err != nil {
			return i, events, coalescedLogs, err
		}

		// Verify commit msg
		if err := bc.engine.VerifyCommitMsg(block); err != nil {
			return i, events, coalescedLogs, fmt.Errorf("commit msg error: %s", err)
		}
...
func (bc *BlockChain) VerifyBlock(block *types.Block) (types.Receipts, []*types.Log, uint64, error) {
	...
	if BadHashes[block.Hash()] {
		return nil, nil, 0, ErrBlacklistedHash
	}
...
func (bc *BlockChain) WriteBlock(block *types.Block) error {
	...
	// Process block using the parent state as reference point.
	receipts, logs, _, err := bc.processor.Process(block, stateDb, bc.vmConfig)
	if err != nil {
		bc.reportBlock(block, receipts, err)
		return err
	}
