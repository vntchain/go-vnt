...
func run(evm *EVM, contract *Contract, input []byte) ([]byte, error) {
	if contract.CodeAddr != nil {
		precompiles := PrecompiledContractsHubble
...
// EVM is the VNT Virtual Machine base object and provides
...
func NewEVM(ctx Context, statedb inter.StateDB, chainConfig *params.ChainConfig, vmConfig Config) *EVM {
	evm := &EVM{
		Context:     ctx,
		StateDB:     statedb,
		vmConfig:    vmConfig,
...
func (evm *EVM) DelegateCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	ctr := NewContract(caller, to, nil, gas).AsDelegate()
...
func (evm *EVM) GetStateDb() inter.StateDB {
	return evm.StateDB
}

func (evm *EVM) GetContext() Context {
	return evm.Context
}
