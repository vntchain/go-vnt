...
func TestHubbleSigning(t *testing.T) {
	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)

	signer := NewHubbleSigner(big.NewInt(18))
...
func TestHubbleChainId(t *testing.T) {
	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)
	...
	if tx.ChainId().Int64() != 18 {
		t.Error("expected chain id to be 18 got", tx.ChainId())
	}
}

func TestHubbleSigningVitalik(t *testing.T) {
	...
	for i, test := range []struct {
		...
		signer := NewHubbleSigner(big.NewInt(1))
...
func TestChainId(t *testing.T) {
	...
	var err error
	tx, err = SignTx(tx, NewHubbleSigner(big.NewInt(1)), key)
	if err != nil {
		t.Fatal(err)
	}

	_, err = Sender(NewHubbleSigner(big.NewInt(2)), tx)
	if err != ErrInvalidChainId {
		t.Error("expected error:", ErrInvalidChainId)
	}

	_, err = Sender(NewHubbleSigner(big.NewInt(1)), tx)
	if err != nil {
