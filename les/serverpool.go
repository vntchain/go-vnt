...
type serverPool struct {
	...
	entries              map[libp2p.ID]*poolEntry
	lock                 sync.Mutex
...
func newServerPool(db vntdb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
		entries:      make(map[libp2p.ID]*poolEntry),
...
func (pool *serverPool) start(server *vntp2p.Server) {
	pool.server = server

	// pool.topic = topic
	// pool.dbKey = append([]byte("serverPool/"), []byte(topic)...)

	pool.dbKey = append([]byte("serverPool/"))
	pool.wg.Add(1)
	pool.loadNodes()
...
func (pool *serverPool) connect(p *peer, ip net.IP, port uint16) *poolEntry {
	...
	entry := pool.entries[p.RemoteID()]
	if entry == nil {
		entry = pool.findOrNewNode(p.RemoteID(), ip, port)
	}
...
func (pool *serverPool) registered(entry *poolEntry) {
	log.Debug("Registered new entry", "vnode", entry.id)
	pool.lock.Lock()
	defer pool.lock.Unlock()
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	log.Debug("Disconnected old entry", "vnode", entry.id)
	pool.lock.Lock()
	defer pool.lock.Unlock()
...
func (pool *serverPool) eventLoop() {
	...
	for {
		...
		// 	pool.lock.Lock()
		// 	entry := pool.findOrNewNode(vntp2p.NodeID(node.ID), node.IP, node.TCP)
		// 	pool.updateCheckDial(entry)
		// 	pool.lock.Unlock()
...
func (pool *serverPool) findOrNewNode(id libp2p.ID, ip net.IP, port uint16) *poolEntry {
	now := mclock.Now()
