...
import (
	...
	"github.com/vntchain/go-vnt/les/flowcontrol"
...
func sendRequest(w vntp2p.MsgWriter, msgcode vntp2p.MessageType, reqID, cost uint64, data interface{}) error {
	type req struct {
		ReqID uint64
		Data  interface{}
	}
	return vntp2p.Send(w, "les", msgcode, req{reqID, data})
}

func sendResponse(w vntp2p.MsgWriter, msgcode vntp2p.MessageType, reqID, bv uint64, data interface{}) error {
	type resp struct {
		ReqID, BV uint64
		Data      interface{}
	}
	return vntp2p.Send(w, "les", msgcode, resp{reqID, bv, data})
}
...
func (p *peer) SendAnnounce(request announceData) error {
	return vntp2p.Send(p.rw, "les", AnnounceMsg, request)
}
...
func (p *peer) SendTxs(reqID, cost uint64, txs types.Transactions) error {
	...
	switch p.version {
	case lpv1:
		return vntp2p.Send(p.rw, "les", SendTxMsg, txs) // old message format does not include reqID
	case lpv2:
...
func (p *peer) sendReceiveHandshake(sendList keyValueList) (keyValueList, error) {
	...
	go func() {
		errc <- vntp2p.Send(p.rw, "les", StatusMsg, sendList)
	}()
	...
	if msg.Body.Type != StatusMsg {
		return nil, errResp(ErrNoStatusMsg, "first msg has code %x (!= %x)", msg.Body.Type, StatusMsg)
	}

	size := msg.GetBodySize()
	if size > ProtocolMaxMsgSize {
		return nil, errResp(ErrMsgTooLarge, "%v > %v", size, ProtocolMaxMsgSize)
	}
...
func (ps *peerSet) Unregister(id libp2p.ID) error {
	...
	if p, ok := ps.peers[id]; !ok {
		...
		p.sendQueue.quit()
		p.Peer.Disconnect(vntp2p.DiscUselessPeer)
		return nil
	}
}

// AllPeerIDs returns a list of all registered peer IDs
func (ps *peerSet) AllPeerIDs() []libp2p.ID {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	res := make([]libp2p.ID, len(ps.peers))
	idx := 0
