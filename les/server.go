...
func NewLesServer(vnt *vnt.VNT, config *vnt.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(vnt.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, vnt.EventMux(), vnt.Engine(), newPeerSet(), vnt.BlockChain(), vnt.TxPool(), vnt.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	// 	lesTopics[i] = lesTopic(vnt.BlockChain().Genesis().Hash(), pv)
	// }
	...
	srv := &LesServer{
		...
		quitSync:        quitSync,
		// lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(vnt.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(vnt.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(vnt.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(vnt.ChainDb())
	return srv, nil
}

func (s *LesServer) Protocols() []vntp2p.Protocol {
	return s.protocolManager.SubProtocols
}

// Start starts the LES server
func (s *LesServer) Start(srvr *vntp2p.Server) {
...
func newCostStats(db vntdb.Database) *requestCostStats {
