...
const (
	...
	vntVersion = 63 // equivalent vnt version for the downloader
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light VNT protocol")
	...
	log.Info("Light VNT protocol stopped")
}

func (pm *ProtocolManager) newPeer(pv int, nv uint64, p *vntp2p.Peer, rw vntp2p.MsgReadWriter) *peer {
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	// p.Log().Debug("Light VNT peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light VNT handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light VNT peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light VNT message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light VNT message arrived", "code", msg.Body.Type, "bytes", msg.GetBodySize())

	costs := p.fcCosts[uint64(msg.Body.Type)]
	...
	case GetBlockHeadersMsg:
		...
		pm.server.fcCostStats.update(uint64(msg.Body.Type), query.Amount, rcost)
	...
	case GetBlockBodiesMsg:
		...
		pm.server.fcCostStats.update(uint64(msg.Body.Type), uint64(reqCnt), rcost)
	...
	case GetHeaderProofsMsg:
		...
		trieDb := trie.NewDatabase(vntdb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
		...
		for _, req := range req.Reqs {
			if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
				...
				if root, prefix = pm.getHelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
					auxTrie, _ = trie.New(root, trie.NewDatabase(vntdb.NewTable(pm.chainDb, prefix)))
				}
			}
	...
	default:
		p.Log().Trace("Received unknown message", "code", msg.Body.Type)
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	pm.downloader.RegisterLightPeer(p.id, vntVersion, pc)
}
