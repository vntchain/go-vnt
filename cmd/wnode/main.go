...
import (
	...
	"github.com/vntchain/go-vnt/whisper/mailserver"
...
var (
	...
	argVnode   = flag.String("boot", "", "bootstrap node you want to connect to (e.g. /ip4/.../tcp/30303/....)")
...
func processArgs() {
	...
	const vnodePrefix = "/"
	if len(*argVnode) > 0 {
		if (*argVnode)[:len(vnodePrefix)] != vnodePrefix {
			utils.Fatalf("Error Url")
		}
	}
...
func initialize() {
	...
	if *bootstrapMode {
		...
		*bootstrapMode = true
		...
		if len(*argVnode) == 0 {
			argVnode = scanLineA("Please enter the peer's vnode: ")
		}
		peer := vntp2p.MustParseNode(*argVnode)
...
func startServer() error {
	...
	// fmt.Println(server.NodeInfo().Vnode)
...
func requestExpiredMessagesLoop() {
	...
	peerID := extractIDFromEnode(*argVnode)
...
func extractIDFromEnode(s string) peer.ID {
	n, err := vntp2p.ParseNode(s)
	if err != nil {
		utils.Fatalf("Failed to parse vnode: %s", err)
	}
