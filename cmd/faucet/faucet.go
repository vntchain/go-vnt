...
// faucet is a Vnt faucet backed by a light client.
package main
...
var (
	...
	vntPortFlag = flag.Int("vntport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode vnode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the VNT protocol")
	statsFlag   = flag.String("vntstats", "", "Vntstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of vnts to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for VNT and the faucet")
)

var (
	vntUnit = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Vnts", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	var vnodes []*vntp2p.Node

	for _, boot := range strings.Split(*bootFlag, ",") {
		if vnode, err := vntp2p.ParseNode(boot); err == nil {
	...
	// faucet, err := newFaucet(genesis, *vntPortFlag, *netFlag, *statsFlag, ks, website.Bytes())
	faucet, err := newFaucet(rctx, genesis, *vntPortFlag, vnodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // VNT address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an VNT light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // VNT protocol stack
	client *vntclient.Client   // Client connection to the VNT chain
...
// func newFaucet(genesis *core.Genesis, port int, vnodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
func newFaucet(rctx context.Context, genesis *core.Genesis, port int, vnodes []*vntp2p.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	stack, err := node.New(&node.Config{
		...
		P2P: vntp2p.Config{
			...
			MaxPeers:       25,
			BootstrapNodes: vnodes,
		},
	})
	if err != nil {
		return nil, err
	}
	// Assemble the VNT light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := vnt.DefaultConfig
	...
	// Assemble the vntstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightVnt
			ctx.Service(&serv)
			return vntstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := vntclient.NewClient(api)
...
// close terminates the VNT connection and tears down the faucet.
...
// apiHandler handles requests for Vnt grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, vntUnit),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the VNT address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/vntchain/go-vnt/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), vntUnit)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, vntUnit)
...
// the username, avatar URL and VNT address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for VNT addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No VNT address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the VNT address and profile URL.
...
// returning the username, avatar URL and VNT address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain VNT address,
