...
ADD vntstats.json /vntstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../vnt-net-intelligence-api && pm2 start /vntstats.json)' >  explorer.sh && \
	echo '(cd ../vntchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerVntstats is the configuration file for the vntstats javascript client.
var explorerVntstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.vntchain
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Vntstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	vntstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerVntstats)).Execute(vntstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.vntstats[:strings.Index(config.vntstats, ":")],
		"Secret": config.vntstats[strings.Index(config.vntstats, ":")+1 : strings.Index(config.vntstats, "@")],
		"Host":   config.vntstats[strings.Index(config.vntstats, "@")+1:],
	})
	files[filepath.Join(workdir, "vntstats.json")] = vntstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.vntchain"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		vntstats: infos.envvars["STATS"],
	}
	return stats, nil
}
