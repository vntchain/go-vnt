...
type config struct {
	...
	vntstats  string   // Vntstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // VNT services known to be running on servers
...
func (w *wizard) readNodeInfo() string {
	for {
		fmt.Printf(">")
		if text, err := w.in.ReadString('\n'); err != nil {
			log.Error("Failed to read user input, please retry")
			continue
		} else {
			...
			if _, err = vntp2p.ParseNode(text); err != nil {
				log.Error("ParseNode err, please retry", "err", err)
				continue
			} else {
				return text
			}
...
// converts it to an VNT address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by vntstats, which compares IPs textually, not by value.
