// Copyright 2017 The go-Vnt Authors
...
import (
	...
	"github.com/vntchain/go-vnt/accounts/usbwallet/internal/trezor"
...
// or whether the VNT app was not started on it.
...
// VNT address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.VntAddress)
	if _, err := w.trezorExchange(&trezor.VntGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.VntTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.VntTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
