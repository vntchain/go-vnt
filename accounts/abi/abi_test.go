...
func TestABI_UnpackInput(t *testing.T) {
	const abiJSON = `[{"constant":false,"inputs":[{"name":"memo","type":"bytes"}],"name":"receive","outputs":[],"payable":true,"stateMutability":"payable","type":"function"},{"anonymous":false,"inputs":[{"indexed":false,"name":"sender","type":"address"},{"indexed":false,"name":"amount","type":"uint256"},{"indexed":false,"name":"memo","type":"bytes"}],"name":"received","type":"function"}]`
	abi, err := JSON(strings.NewReader(abiJSON))
	if err != nil {
		t.Fatal(err)
	}
	...
	data, err := abi.Pack("received", ev.Sender, ev.Amount, ev.Memo)
	if err != nil {
		t.Error(err)
	}

	var unpackEv Received
	err = abi.UnpackInput(&unpackEv, "received", data[4:])
	if err != nil {
		t.Error(err)
	} else {
		t.Logf("len(data): %d; received : %+v", len(data), unpackEv)
	}
	if !bytes.Equal(unpackEv.Sender[:], ev.Sender[:]) {
		t.Errorf("the sender before and after pack is different. Before: 0x%x, after: 0x%x", ev.Sender, unpackEv.Sender)
	}
	if !bytes.Equal(unpackEv.Memo, ev.Memo) {
		t.Errorf("the memo before and after pack is different. Before: 0x%x, after: 0x%x", ev.Memo, unpackEv.Memo)
	}
	if !bytes.Equal(unpackEv.Amount.Bytes(), ev.Amount.Bytes()) {
		t.Errorf("the amount before and after pack is different.Before: %d, after: %d", ev.Amount, unpackEv.Amount)
	}
}
