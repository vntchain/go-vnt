...
type SimulatedBackend struct {
	database   vntdb.Database   // In memory database to store our testing data
	blockchain *core.BlockChain // VNT blockchain to handle the consensus
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call hubble.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call hubble.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call hubble.CallMsg) (uint64, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call hubble.CallMsg, block *types.Block, stateObj *state.StateDB) ([]byte, uint64, bool, error) {
	...
	from := stateObj.GetOrNewStateObject(call.From)
...
func (b *SimulatedBackend) FilterLogs(ctx context.Context, query hubble.FilterQuery) ([]types.Log, error) {
...
func (b *SimulatedBackend) SubscribeFilterLogs(ctx context.Context, query hubble.FilterQuery, ch chan<- types.Log) (hubble.Subscription, error) {
...
func (fb *filterBackend) ChainDb() vntdb.Database  { return fb.db }
