...
import "github.com/vntchain/go-vnt/accounts/abi"
...
type tmplContract struct {
	...
	InputBin    string                 // Optional VM bytecode used to denetare deploy code from
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		// Deploy{{.Type}} deploys a new VNT contract, binding an instance of {{.Type}} to it.
	...
	// {{.Type}} is an auto generated Go binding around an VNT contract.
	type {{.Type}} struct {
	...
	// {{.Type}}Caller is an auto generated read-only Go binding around an VNT contract.
	type {{.Type}}Caller struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Transactor is an auto generated write-only Go binding around an VNT contract.
	type {{.Type}}Transactor struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Filterer is an auto generated log filtering Go binding around an VNT contract events.
	type {{.Type}}Filterer struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Session is an auto generated Go binding around an VNT contract,
	...
	// {{.Type}}CallerSession is an auto generated read-only Go binding around an VNT contract,
	...
	// {{.Type}}TransactorSession is an auto generated write-only Go binding around an VNT contract,
	...
	// {{.Type}}Raw is an auto generated low-level Go binding around an VNT contract.
	type {{.Type}}Raw struct {
	  Contract *{{.Type}} // Generic contract binding to access the raw methods on
	}

	// {{.Type}}CallerRaw is an auto generated low-level read-only Go binding around an VNT contract.
	type {{.Type}}CallerRaw struct {
		Contract *{{.Type}}Caller // Generic read-only contract binding to access the raw methods on
	}

	// {{.Type}}TransactorRaw is an auto generated low-level write-only Go binding around an VNT contract.
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		{{if .InputBin}}
			...
			// deploy deploys a new VNT contract, binding an instance of {{.Type}} to it.
			public static {{.Type}} deploy(TransactOpts auth, VNTClient client{{range .Constructor.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Gvnt.newInterfaces({{(len .Constructor.Inputs)}});
				{{range $index, $element := .Constructor.Inputs}}
				  args.set({{$index}}, Gvnt.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}
				return new {{.Type}}(Gvnt.deployContract(auth, ABI, BYTECODE, client, args));
			}
		...
		// VNT address where this contract is located at.
		public final Address Address;

		// VNT transaction in which this contract was deployed (if known!).
		...
		public {{.Type}}(Address address, VNTClient client) throws Exception {
			this(Gvnt.bindContract(address, ABI, client));
		}
		...
		{{range .Calls}}
			...
			public {{if gt (len .Normalized.Outputs) 1}}{{capitalise .Normalized.Name}}Results{{else}}{{range .Normalized.Outputs}}{{bindtype .Type}}{{end}}{{end}} {{.Normalized.Name}}(CallOpts opts{{range .Normalized.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Gvnt.newInterfaces({{(len .Normalized.Inputs)}});
				{{range $index, $item := .Normalized.Inputs}}args.set({{$index}}, Gvnt.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}

				Interfaces results = Gvnt.newInterfaces({{(len .Normalized.Outputs)}});
				{{range $index, $item := .Normalized.Outputs}}Interface result{{$index}} = Gvnt.newInterface(); result{{$index}}.setDefault{{namedtype (bindtype .Type) .Type}}(); results.set({{$index}}, result{{$index}});
				{{end}}

				if (opts == null) {
					opts = Gvnt.newCallOpts();
				}
