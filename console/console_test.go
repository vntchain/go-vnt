...
func newTester(t *testing.T, confOverride func(*vnt.Config)) *tester {
	...
	// Create a networkless protocol stack and start an VNT service within
	p2pCfg := vntp2p.Config{
		ListenAddr: "0030421",
	}
	stack, err := node.New(&node.Config{DataDir: workspace, P2P: p2pCfg, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
