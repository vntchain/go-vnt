...
// Service implements an VNT netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *vntp2p.Server   // Peer-to-peer server to retrieve networking infos
	vnt    *vnt.VNT         // Full VNT service if monitoring a full node
	les    *les.LightVnt    // Light VNT service if monitoring a light node
...
func (s *Service) Protocols() []vntp2p.Protocol { return nil }
...
func (s *Service) Start(server *vntp2p.Server) error {
	s.server = server
	go s.loop()
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["vnt"]; info != nil {
		network = fmt.Sprintf("%d", info.(*vnt.NodeInfo).Network)
		protocol = fmt.Sprintf("vnt/%d", vnt.ProtocolVersions[0])
	} else {
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.vnt != nil {
		...
		if block == nil {
			block = s.vnt.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.vnt.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.vnt != nil {
			head = s.vnt.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.vnt != nil {
			block = s.vnt.BlockChain().GetBlockByNumber(number)
		} else {
